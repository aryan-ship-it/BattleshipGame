...
//player1 route link to send request to the server 
let playerroute = 'http://127.0.0.1:3000/player1'

//helper function which sends parameters status,coordinates and style through the URL 
function StatusParamaterSender(status,coordinates,style){
    let url = new URL(playerroute) //url which sends request through the player1Route
    let parameter = url.searchParams; //access the parameters in the URL
    parameter.set('username',username); //set username parameter in the URL
    parameter.set('gamecoordinates',coordinates); //set gamecoordinates parameter in the URL
    parameter.set('gamestatus',status); //set gamestatus parameter in the URL
    parameter.set('style',style); //set style parameter which further is used to record the moves and gamestatus
